...
class PHPExcel_Reader_Excel5 implements PHPExcel_Reader_IReader
{
	...
	private function _readFont()
	{
		...
		if (!$this->_readDataOnly) {
			...
			$isItalic = (2 & $this->_GetInt2d($recordData, 2)) >> 1;

			if ($isItalic) {
				$objFont->setItalic(true);
			}

			$isStrike = (8 & $this->_GetInt2d($recordData, 2)) >> 3;
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				if ($bordersLeftStyle = $this->_mapBorderStyle((15 & $this->_GetInt4d($recordData, 10)) >> 0)) {
					$objStyle->getBorders()->getLeft()->setBorderStyle($bordersLeftStyle);
				}

				if ($bordersRightStyle = $this->_mapBorderStyle((240 & $this->_GetInt4d($recordData, 10)) >> 4)) {
					$objStyle->getBorders()->getRight()->setBorderStyle($bordersRightStyle);
				}

				if ($bordersTopStyle = $this->_mapBorderStyle((3840 & $this->_GetInt4d($recordData, 10)) >> 8)) {
					$objStyle->getBorders()->getTop()->setBorderStyle($bordersTopStyle);
				}

				if ($bordersBottomStyle = $this->_mapBorderStyle((61440 & $this->_GetInt4d($recordData, 10)) >> 12)) {
					$objStyle->getBorders()->getBottom()->setBorderStyle($bordersBottomStyle);
				}
/* [31m * TODO SEPARATE[0m */
/* [31m * TODO SEPARATE[0m */

				$objStyle->getBorders()->getLeft()->colorIndex = (8323072 & $this->_GetInt4d($recordData, 10)) >> 16;
				$objStyle->getBorders()->getRight()->colorIndex = (1065353216 & $this->_GetInt4d($recordData, 10)) >> 23;
				$diagonalDown = ((1073741824 & $this->_GetInt4d($recordData, 10)) >> 30 ? true : false);
				$diagonalUp = ((2147483648 & $this->_GetInt4d($recordData, 10)) >> 31 ? true : false);
...
/* [31m * TODO SEPARATE[0m */
				...
				$objStyle->getBorders()->getTop()->colorIndex = (127 & $this->_GetInt4d($recordData, 14)) >> 0;
				$objStyle->getBorders()->getBottom()->colorIndex = (16256 & $this->_GetInt4d($recordData, 14)) >> 7;
				$objStyle->getBorders()->getDiagonal()->colorIndex = (2080768 & $this->_GetInt4d($recordData, 14)) >> 14;

				if ($bordersDiagonalStyle = $this->_mapBorderStyle((31457280 & $this->_GetInt4d($recordData, 14)) >> 21)) {
					$objStyle->getBorders()->getDiagonal()->setBorderStyle($bordersDiagonalStyle);
				}

				if ($fillType = $this->_mapFillPattern((4227858432 & $this->_GetInt4d($recordData, 14)) >> 26)) {
					$objStyle->getFill()->setFillType($fillType);
				}
/* [31m * TODO SEPARATE[0m */
/* [31m * TODO SEPARATE[0m */

				$objStyle->getFill()->startcolorIndex = (127 & $this->_GetInt2d($recordData, 18)) >> 0;
				$objStyle->getFill()->endcolorIndex = (16256 & $this->_GetInt2d($recordData, 18)) >> 7;
			}
			else {
...
/* [31m * TODO SEPARATE[0m */
				...
				$objStyle->getFill()->setFillType($this->_mapFillPattern((4128768 & $borderAndBackground) >> 16));
				$objStyle->getBorders()->getBottom()->setBorderStyle($this->_mapBorderStyle((29360128 & $borderAndBackground) >> 22));
				$objStyle->getBorders()->getBottom()->colorIndex = (4261412864 & $borderAndBackground) >> 25;
				$borderLines = $this->_GetInt4d($recordData, 12);
				$objStyle->getBorders()->getTop()->setBorderStyle($this->_mapBorderStyle((7 & $borderLines) >> 0));
				$objStyle->getBorders()->getLeft()->setBorderStyle($this->_mapBorderStyle((56 & $borderLines) >> 3));
				$objStyle->getBorders()->getRight()->setBorderStyle($this->_mapBorderStyle((448 & $borderLines) >> 6));
	...
	private function _readXfExt()
	{
		...
		if (!$this->_readDataOnly) {
			...
			while ($offset < $length) {
				...
				switch ($extType) {
				case 4:
					$xclfType = $this->_GetInt2d($extData, 0);
	...
	private function _readSheetPr()
	{
		...
		$recordData = substr($this->_data, $this->_pos + 4, $length);
		$this->_pos += 4 + $length;
		$isSummaryBelow = (64 & $this->_GetInt2d($recordData, 0)) >> 6;
		$this->_phpSheet->setShowSummaryBelow($isSummaryBelow);
		$isSummaryRight = (128 & $this->_GetInt2d($recordData, 0)) >> 7;
		$this->_phpSheet->setShowSummaryRight($isSummaryRight);
		$this->_isFitToPages = (bool) ((256 & $this->_GetInt2d($recordData, 0)) >> 8);
	}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			$isPortrait = (2 & $this->_GetInt2d($recordData, 10)) >> 1;
			$isNotInit = (4 & $this->_GetInt2d($recordData, 10)) >> 2;
	...
	private function _readProtect()
	{
		...
		$bool = (1 & $this->_GetInt2d($recordData, 0)) >> 0;
	...
	private function _readColInfo()
	{
		...
		if (!$this->_readDataOnly) {
			...
			$isHidden = (1 & $this->_GetInt2d($recordData, 8)) >> 0;
			$level = (1792 & $this->_GetInt2d($recordData, 8)) >> 8;
			$isCollapsed = (4096 & $this->_GetInt2d($recordData, 8)) >> 12;
	...
	private function _readRow()
	{
		...
		if (!$this->_readDataOnly) {
			...
			$height = (32767 & $this->_GetInt2d($recordData, 6)) >> 0;
			$useDefaultHeight = (32768 & $this->_GetInt2d($recordData, 6)) >> 15;

			if (!$useDefaultHeight) {
				$this->_phpSheet->getRowDimension($r + 1)->setRowHeight($height / 20);
			}

			$level = (7 & $this->_GetInt4d($recordData, 12)) >> 0;
			$this->_phpSheet->getRowDimension($r + 1)->setOutlineLevel($level);
			$isCollapsed = (16 & $this->_GetInt4d($recordData, 12)) >> 4;
			$this->_phpSheet->getRowDimension($r + 1)->setCollapsed($isCollapsed);
			$isHidden = (32 & $this->_GetInt4d($recordData, 12)) >> 5;
			$this->_phpSheet->getRowDimension($r + 1)->setVisible(!$isHidden);
			$hasExplicitFormat = (128 & $this->_GetInt4d($recordData, 12)) >> 7;
			$xfIndex = (268369920 & $this->_GetInt4d($recordData, 12)) >> 16;
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			$isFileLinkOrUrl = (1 & $this->_GetInt2d($recordData, 28)) >> 0;
			$isAbsPathOrUrl = (1 & $this->_GetInt2d($recordData, 28)) >> 1;
			$hasDesc = (20 & $this->_GetInt2d($recordData, 28)) >> 2;
			$hasText = (8 & $this->_GetInt2d($recordData, 28)) >> 3;
			$hasFrame = (128 & $this->_GetInt2d($recordData, 28)) >> 7;
			$isUNC = (256 & $this->_GetInt2d($recordData, 28)) >> 8;
			$offset = 32;
	...
	private function _readBIFF8CellAddress($cellAddressStructure)
	{
		...
		$column = PHPExcel_Cell::stringFromColumnIndex(255 & $this->_GetInt2d($cellAddressStructure, 2));

		if (!(16384 & $this->_GetInt2d($cellAddressStructure, 2))) {
			$column = '$' . $column;
		}

		if (!(32768 & $this->_GetInt2d($cellAddressStructure, 2))) {
			$row = '$' . $row;
		}
	...
	private function _readBIFF8CellAddressB($cellAddressStructure, $baseCell = 'A1')
	{
		...
		$row = $this->_GetInt2d($cellAddressStructure, 0) + 1;
		$colIndex = 255 & $this->_GetInt2d($cellAddressStructure, 2);
	...
	private function _readBIFF8CellRangeAddress($subData)
	{
		$fr = $this->_GetInt2d($subData, 0) + 1;
		$lr = $this->_GetInt2d($subData, 2) + 1;
		$fc = PHPExcel_Cell::stringFromColumnIndex(255 & $this->_GetInt2d($subData, 4));

		if (!(16384 & $this->_GetInt2d($subData, 4))) {
			$fc = '$' . $fc;
		}

		if (!(32768 & $this->_GetInt2d($subData, 4))) {
			$fr = '$' . $fr;
		}

		$lc = PHPExcel_Cell::stringFromColumnIndex(255 & $this->_GetInt2d($subData, 6));

		if (!(16384 & $this->_GetInt2d($subData, 6))) {
			$lc = '$' . $lc;
		}

		if (!(32768 & $this->_GetInt2d($subData, 6))) {
			$lr = '$' . $lr;
		}
	...
	private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
	{
		...
		$fcIndex = 255 & $this->_GetInt2d($subData, 4);
		...
		$lcIndex = 255 & $this->_GetInt2d($subData, 6);
	...
	private function _readColor($color)
	{
		if (($color <= 7) || (64 <= $color)) {
			...
			if (isset($this->_palette) && isset($this->_palette[$color - 8])) {
