...
class ntlm_sasl_client_class
{
	...
	public function TypeMsg1($domain, $workstation)
	{
		...
		return "NTLMSSP\x00" . "\x01\x00\x00\x00" . "\x072\x00\x00" . pack('v', $domain_length) . pack('v', $domain_length) . pack('V', $domain_offset) . pack('v', $workstation_length) . pack('v', $workstation_length) . pack('V', $workstation_offset) . $workstation . $domain;
	}
	...
	public function NTLMResponse($challenge, $password)
	{
		...
		for ($third = 0; $third < 21; $third += 7) {
			$packed = '';

			for ($p = $third; $p < ($third + 7); $p++) {
	...
	public function TypeMsg3($ntlm_response, $user, $domain, $workstation)
	{
		...
		return "NTLMSSP\x00" . "\x03\x00\x00\x00" . pack('v', $lm_length) . pack('v', $lm_length) . pack('V', $lm_offset) . pack('v', $ntlm_length) . pack('v', $ntlm_length) . pack('V', $ntlm_offset) . pack('v', $domain_length) . pack('v', $domain_length) . pack('V', $domain_offset) . pack('v', $user_length) . pack('v', $user_length) . pack('V', $user_offset) . pack('v', $workstation_length) . pack('v', $workstation_length) . pack('V', $workstation_offset) . pack('v', $session_length) . pack('v', $session_length) . pack('V', $session_offset) . "\x01\x02\x00\x00" . $domain_unicode . $user_unicode . $workstation_unicode . $lm . $ntlm;
	}
