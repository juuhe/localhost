	...
	public function fetch($URI)
	{
		...
		case 'https':
			if (!extension_loaded('openssl')) {
				trigger_error('openssl extension required for HTTPS', 256);
				exit();
			}
	...
	public function _httprequest($url, $fp, $URI, $http_method, $content_type = '', $body = '')
	{
		...
		if ($this->use_gzip) {
			if (function_exists('gzinflate')) {
				...
				trigger_error('use_gzip is on, but PHP was built without zlib support.' . '  Requesting file(s) without gzip encoding.', 1024);
			}
		}
	...
	public function _connect(&$fp)
	{
		if (!empty($this->proxy_host) && !empty($this->proxy_port)) {
			...
			if ($this->scheme == 'https') {
				trigger_error('HTTPS connections over proxy are currently not supported', 256);
				exit();
			}
		}
		else {
